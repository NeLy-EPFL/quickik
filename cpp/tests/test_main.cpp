// Tests for the FastIK C++ bindings, mirroring tests/solver_test.rs and
...
rust::Box<fastik::KinematicTree> two_joint_chain() {
  return fastik::kinematic_tree_from_json_str(kTwoJointChainJson);
}
...
std::vector<fastik::KeypointObservation> observations_for(float a1, float a2) {
  std::vector<fastik::KeypointObservation> obs;
  for (auto &pos : two_link_positions(a1, a2)) {
    obs.push_back(fastik::keypoint_position_3d(pos, 1.0f));
  }
  return obs;
}
...
fastik::SolverConfig no_prior_config() {
...
bool test_malformed_json_throws() {
  bool ok = true;
  try {
    fastik::kinematic_tree_from_json_str("not valid json");
...
bool test_recovers_pose_from_3d_observations() {
  ...
  auto solver = fastik::new_solver(*tree, no_prior_config(), fastik::no_mapper());
...
bool test_position2d_observation_on_mapperless_solver_throws() {
  ...
  for (size_t i = 0; i < tree->n_joints(); i++) observations.push_back(fastik::keypoint_missing());
  observations[1] = fastik::keypoint_position_2d({1.0f, 0.0f}, 1.0f);
...
bool test_recovers_pose_from_xyview_observations() {
  ...
  for (auto &pos : positions) {
    observations.push_back(fastik::keypoint_position_2d({pos[0], pos[1]}, 1.0f));
  }

  auto state = fastik::state_neutral_pose(*tree);
  auto solver = fastik::new_solver(*tree, no_prior_config(), fastik::xyview_mapper());
  solver->solve(*state, slice_of(observations));
...
bool test_recovers_pose_from_camera_observations() {
  ...
  for (auto &pos : positions) {
    ...
    observations.push_back(fastik::keypoint_position_2d({u, v}, 1.0f));
  }

  auto state = fastik::state_neutral_pose(*tree);
  auto solver = fastik::new_solver(*tree, no_prior_config(), fastik::camera_mapper(camera));
  solver->solve(*state, slice_of(observations));
...
bool test_missing_observations_leave_state_at_neutral_prior() {
  ...
  for (size_t i = 0; i < tree->n_joints(); i++) {
    observations.push_back(fastik::keypoint_missing());
  }
...
bool test_config_can_be_tuned_between_solve_calls() {
  ...
  fastik::SolverConfig config = solver->config();
...
bool test_solve_respects_joint_limits() {
  ...
  std::vector<fastik::KeypointObservation> observations = {
      fastik::keypoint_missing(),
      fastik::keypoint_position_3d({1.0f, 0.0f, 0.0f}, 1.0f),
      fastik::keypoint_position_3d({2.0f, 0.0f, 0.0f}, 1.0f),
      fastik::keypoint_position_3d({2.3624f, 0.9320f, 0.0f}, 1.0f),
  };
...
bool test_sequence_solver_warm_start_converges_faster() {
  bool ok = true;
  auto tree = two_joint_chain();
  fastik::SolverConfig config = no_prior_config();
  config.n_iterations = 1;
  auto target = observations_for(0.4f, 0.3f);

  auto cold = fastik::new_sequence_solver(*tree, config, fastik::no_mapper());
  cold->solve_frame(slice_of(target));
  float cold_error = std::abs(cold->state()->dof_angles()[0] - 0.4f);

  auto warm = fastik::new_sequence_solver(*tree, config, fastik::no_mapper());
...
bool test_solve_sequence_returns_one_state_per_frame() {
  bool ok = true;
  auto tree = two_joint_chain();
  auto solver = fastik::new_sequence_solver(*tree, fastik::default_solver_config(), fastik::no_mapper());
...
bool test_solve_sequence_segmented_parallel_reconstructs_smooth_trajectory() {
  ...
  fastik::SegmentedSolveConfig segmented_config{10, 3, 0.05f};
  auto states = fastik::solve_sequence_segmented_parallel(
      *tree, fastik::default_solver_config(), slice_of(flat), tree->n_joints(), segmented_config, fastik::no_mapper());
