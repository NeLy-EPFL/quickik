...
fn camera_projects_point_directly_in_front_to_principal_point() {
    ...
    let (pos2d, _) = camera.project_3d_to_2d(&Vector3::new(0.0, 0.0, 0.0), &jacobian_world3d);
...
fn camera_jacobian_matches_finite_differences() {
    ...
    let (baseline_2d, analytical_jac) = camera.project_3d_to_2d(&pos_world3d, &jacobian_world3d);
    ...
    for axis in 0..3 {
        ...
        perturbed[axis] += eps;
        let (perturbed_2d, _) = camera.project_3d_to_2d(&perturbed, &jacobian_world3d);
