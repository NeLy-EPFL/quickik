...
//! fixes frames-per-thread at `fastik_benchmark::perf::SEGMENT_LEN` (one
...
//! `n_threads` entry in `../plot/results/fastik-scaling.json` (a JSON array,
...
fn write_results_json(n_threads: usize, total_frames: usize, elapsed_s: f64, throughput_fps: f64) {
    ...
    let out_path = out_dir.join("fastik-scaling.json");
    ...
    std::fs::write(&out_path, serde_json::to_string_pretty(&points).unwrap())
        .expect("failed to write ../plot/results/fastik-scaling.json");
}
