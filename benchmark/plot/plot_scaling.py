"""Plots fastik-scaling's sweep (results/fastik-scaling.json -- a JSON array
of {n_threads, total_frames, elapsed_s, throughput_fps} points, one per
thread count -- written by benchmark/fastik_scaling, see its own README and
...
Note: the underlying fastik_scaling test is a *weak*-scaling design (work
per thread is fixed at one 200-frame segment; total work grows with thread
count -- see fastik_scaling's own module docs), not strong scaling (fixed
...
def load_points():
    path = RESULTS_DIR / "fastik-scaling.json"
    if not path.exists():
        return []
...
def print_table(points):
    if not points:
        print(f"No weak-scaling data found in {RESULTS_DIR}/fastik-scaling.json. Run fastik_scaling/run_sweep.sh first.")
        return
...
def plot_chart(points):
    ...
    ax.plot(n_threads, speedup, marker="o", color=COLOR, label="FastIK (Rust)")
